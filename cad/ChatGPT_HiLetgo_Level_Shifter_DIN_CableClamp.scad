...
//   5. The cradle is wider than the original 6 mm cable clamp so it physically
//      supports the full 12.6 mm board width rather than merely using the original profile.
...
slot_rear_y = pcb_rear_y + pcb_face_clear;
slot_front_y = pcb_front_y - pcb_face_clear;
...
cradle_width_x = pcb_x + 2*cradle_side_margin;
...
module pcb_cradle(){
    xw=cradle_width_x;
    translate([0,(spine_front_y+slot_front_y-front_lip_t-lead_in)/2,(channel_inner_z+channel_outer_z)/2])cube([xw,spine_front_y-(slot_front_y-front_lip_t-lead_in),channel_outer_z-channel_inner_z],center=true);
    translate([0,(spine_front_y+slot_rear_y)/2,channel_inner_z-edge_overlap/2])cube([xw,spine_front_y-slot_rear_y,edge_overlap],center=true);
    translate([0,slot_front_y-front_lip_t/2,channel_inner_z-edge_overlap/2])cube([xw,front_lip_t,edge_overlap],center=true);
    hull(){translate([0,slot_front_y-front_lip_t-lead_in+0.05,channel_inner_z-0.15])cube([xw,0.10,0.30],center=true);translate([0,slot_front_y-front_lip_t+0.05,channel_inner_z-edge_overlap/2])cube([xw,0.10,edge_overlap],center=true);}
    translate([0,(spine_front_y+slot_front_y-front_lip_t-lead_in)/2,-(channel_inner_z+channel_outer_z)/2])cube([xw,spine_front_y-(slot_front_y-front_lip_t-lead_in),channel_outer_z-channel_inner_z],center=true);
    translate([0,(spine_front_y+slot_rear_y)/2,-channel_inner_z+edge_overlap/2])cube([xw,spine_front_y-slot_rear_y,edge_overlap],center=true);
    translate([0,slot_front_y-front_lip_t/2,-channel_inner_z+edge_overlap/2])cube([xw,front_lip_t,edge_overlap],center=true);
    hull(){translate([0,slot_front_y-front_lip_t-lead_in+0.05,-channel_inner_z+0.15])cube([xw,0.10,0.30],center=true);translate([0,slot_front_y-front_lip_t+0.05,-channel_inner_z+edge_overlap/2])cube([xw,0.10,edge_overlap],center=true);}
    rib_h=2.2;
    for(sz=[-1,1])translate([0,(slot_rear_y+spine_front_y)/2,sz*(board_half_z-rib_h/2)])cube([mount_width,spine_front_y-slot_rear_y,rib_h],center=true);
}
...
if(show_reference_pcb)%translate([0,(pcb_rear_y+pcb_front_y)/2,0])cube([pcb_x,pcb_t,pcb_z],center=true);
